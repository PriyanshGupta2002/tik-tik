...
const Discover = () => {
  ...
  return (
    <div className='xl:border-b-2 xl:border-gray-200 pb-6'>
        <h2 className='text-gray-500 font-semibold m-3 mt-4 hidden xl:block'>
            Popular Topics
        </h2>
        <div className='flex gap-3 flex-wrap'>
            {topics.map((item)=>{return <Link href={`/?topic=${item.name}`} key={item.name} ><div className={topic===item.name?activeTopicStyle:topicStyle}> <span className='font-bold text-2xl xl:text-md'>{item.icon}</span> <span className='font-medium text-md hidden xl:block capitalize'>{item.name} </span></div></Link> })}
        </div>
    </div>
  )
}
